subroutine init_phi(lo, hi, phi, philo, phihi, dx, prob_lo, prob_hi, k_freq, Nprob) bind(C, name="init_phi")
  ...
  integer          :: i,j, i_quad, j_quad, freq_iter, max_freq_iter
        ...
        do j = lo(2), hi(2)
         ...
         do i = lo(1), hi(1) ! Went philo - phihi before
            ...
            x_quad = x + dx(1)*gauss_nodeFrac(i_quad)
                ...
                do freq_iter = 1,max_freq_iter
                    phi(i,j) = phi(i,j)+ gauss_weights(j_quad)*gauss_weights(i_quad)* &
                        (1.d0/dble(max_freq_iter))*cos(freq_iter*k_freq*(x_quad))*cos(freq_iter*k_freq*(y_quad))
                end do
            end do
            end do
         end do
            ...
            do i = lo(1), hi(1)
                ...
                do j_quad = 0,2
                    ...
                    do i_quad = 0,2
                        ...
                        do freq_iter = 1,max_freq_iter
                        phi(i,j) = phi(i,j)+ gauss_weights(j_quad)*gauss_weights(i_quad)* &
                        (1.d0/dble(max_freq_iter))*sin(freq_iter*k_freq*(x_quad))*sin(freq_iter*k_freq*(y_quad))
                        end do
                    end do
                end do
            end do
        ...
        !print*, y_quad
            do i_quad = 0,4
               ...
               case(1)
                ...
                case default
                  print *,'invalid case in init_beta'
               end select
